# 🎲 RollAndRegister

A cross-platform desktop and mobile application converted from the CC316 project web application.

[![Web App](https://img.shields.io/badge/Web_App-Live-orange?style=for-the-badge&logo=googlechrome&logoColor=white)](https://ivanclarklim2006-sudo.github.io/CC316-PROJECT/)
[![Windows](https://img.shields.io/badge/Windows-0078D4?style=for-the-badge&logo=windows&logoColor=white)](#-downloads)
[![Android](https://img.shields.io/badge/Android-3DDC84?style=for-the-badge&logo=android&logoColor=white)](#-downloads)
[![PRE-Release](https://img.shields.io/badge/PRE_Release-V1.1.0_alpha_0.1-blue
)](https://github.com/ivanclarklim2006-sudo/CC316-PROJECT/releases/tag/PRE-RELEASE_TESTING)

---

## 🌐 Try Online First

Want to test the app without downloading anything? You can try out the live web version right in your browser:

👉 **[Launch RollAndRegister Web App](https://ivanclarklim2006-sudo.github.io/CC316-PROJECT/)**

---

## 📥 Downloads

Select your operating system below to download the latest executable binary:

| Platform | Format | Direct Download |
| :--- | :--- | :--- |
| **Web Browser** | Live Demo | [**Try Web Version**](https://ivanclarklim2006-sudo.github.io/CC316-PROJECT/) |
| **Windows 10/11** | Standalone Executable (`.exe`) | [**Download for Windows**](https://github.com/ivanclarklim2006-sudo/CC316-PROJECT/releases/download/PRE-RELEASE_TESTING/Roll.Register.exe) |
| **Android** | Package Installer (`.apk`) | [**Download for Android**](https://github.com/ivanclarklim2006-sudo/CC316-PROJECT/releases/download/PRE-RELEASE_TESTING/RollAndRegister.apk) |

<br/>

### Quick Download Badges

<a href="https://ivanclarklim2006-sudo.github.io/CC316-PROJECT/">
  <img src="https://img.shields.io/badge/Try_Web_Version-Live_Demo-orange?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Try Web Version">
</a>
&nbsp;
<a href="https://github.com/ivanclarklim2006-sudo/CC316-PROJECT/releases/download/PROJECT/RollAndRegister.exe">
  <img src="https://img.shields.io/badge/Download_for_Windows-EXE-0078D4?style=for-the-badge&logo=windows&logoColor=white" alt="Download Windows App">
</a>
&nbsp;
<a href="https://github.com/ivanclarklim2006-sudo/CC316-PROJECT/releases/download/PROJECT/RollandRegister.apk">
  <img src="https://img.shields.io/badge/Download_for_Android-APK-3DDC84?style=for-the-badge&logo=android&logoColor=white" alt="Download Android App">
</a>

---

## ✨ Features

### 🎓 Student & Teacher Directory
- Look up a student by name or LRN to see their current section, school year, and adviser
- Look up a teacher to see every subject and section they hold
- Section history per student — every past section/school year, with the current one flagged, plus optional "Transferred from" tracking for students who came from another school
- Add, edit, and delete student and teacher records, with checkbox multi-select for bulk deletion

### 🔍 Search, Filter & Sort
- Live search by name or ID across the Students and Teachers tabs
- Browse by Section or School year dropdowns, with the active filter shown as a removable chip
- Sort the visible list Alphabetically (A–Z), by Latest added, or Earliest added — remembered per browser

### 📷 Scan-to-Add (OCR)
- Scan a printed class list or ID with your device camera or a photo
- Text recognition runs fully on-device (Tesseract OCR in the browser) — no photos are uploaded anywhere
- Review screen lets you fix, uncheck, or add rows before bulk-saving the scanned students

### ☁️ Sync & Backup
- Optional Google sign-in to sync the directory to your own Drive and pick up where you left off on another device
- "Continue as Guest" mode for local-only use, with an on-screen reminder to back up before clearing browser data
- Manual backup: download a `.json` backup file or import one to restore/transfer data
- Everything autosaves in the browser as you go — no account or server required to use the app

### 🎨 Customization
- Branding panel to upload your school's logo, edit the heading/title/description text, and choose which side the logo sits on
- Light and dark mode, togglable from the header or right on the sign-in screen, with the choice remembered per browser

### 📱 Cross-Platform
- Installable as a Progressive Web App (PWA) with offline support via a service worker
- Packaged as a standalone Windows executable and an Android APK, alongside the live web app
- Responsive layout tuned for both desktop and mobile use

---

## 🚀 Installation Instructions

<details>
<summary><b>💻 Windows Installation</b></summary>

1. Download **`RollAndRegister.exe`** from the links above.
2. Move the `.exe` file to your preferred folder or Desktop.
3. Double-click **`RollAndRegister.exe`** to open and run the app.
</details>

<details>
<summary><b>📱 Android Installation</b></summary>

1. Download **`RollandRegister.apk`** directly to your phone.
2. Open the file manager on your device and tap the downloaded `.apk`.
3. If prompted by Android, grant permission to **Allow installation from unknown sources**.
4. Complete the installation and open **RollAndRegister** from your app drawer.
</details>
